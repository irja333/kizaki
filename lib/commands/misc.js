(async () => {
     switch(true) {
         case /^(menu|help)$/i.test(command): {
             ...
             conn.sendMessage(from, { contentText: menu(m, body, pushname).trim(), footerText: '© IRJA', buttons: [{ buttonId: `${prefix}owner`, buttonText: { displayText: 'Owner' }, type: 1 },{ buttonId: `${prefix}tos`, buttonText: { displayText: 'Cara Penggunaan' }, type: 1 }], headerType: 'IMAGE', imageMessage: prep }, 'buttonsMessage', { quoted: m })
             break
         }
